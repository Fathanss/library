...
export default function BooksPage() {
  ...
  useEffect(() => {
    fetch('https://jsonplaceholder.typicode.com/posts?_limit=12')
      ...
      .then((data: PostApiResponse[]) => {
        const mappedBorrowers: Borrower[] = data.map((item, idx) => ({
          id: item.id,
          full_name: `Borrower ${idx + 1}`,
          book_title: item.title.slice(0, 30),
          time_borrowed: new Date().toISOString().split('T')[0],
          time_return: new Date( Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 7 days later
          no_hp: `0812345678${idx + 1}`, 
        }));
  ...
  const handleOpenCreate = () => {
    setIsEditing(false);
    setFormData({ full_name: '', book_title: '', time_borrowed: '', time_return: '', no_hp: '' });
  ...
  const handleOpenEdit = (borrower: Borrower) => {
    setIsEditing(true);
    setCurrentBookId(borrower.id);
    setFormData({
      full_name: borrower.full_name,
      book_title: borrower.book_title,
      time_borrowed: borrower.time_borrowed,
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (isEditing && currentBookId !== null) {
      setBorrowers(borrowers.map((b) => (b.id === currentBookId ? { ...b, ...formData } : b)));
      Swal.fire({ icon: 'success', title: 'Updated!', text: 'Borrower updated successfully', timer: 1500, showConfirmButton: false });
    } else {
      ...
      Swal.fire({ icon: 'success', title: 'Created!', text: 'Borrower added successfully', timer: 1500, showConfirmButton: false });
    }
  ...
  const handleDelete = (id: number) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire({ icon: 'success', title: 'Deleted!', text: 'Borrower has been deleted.', timer: 1500, showConfirmButton: false });
      }
    });
  };
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/70 border-b border-slate-100 text-xs font-semibold uppercase tracking-wider text-slate-500">
                ...
                <th className="py-3.5 px-6">Time Borrowed</th>
              ...
              {paginatedBorrowers.length > 0 ? (
                paginatedBorrowers.map((borrower) => (
                  <tr key={borrower.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-4 px-6 text-slate-600">{borrower.book_title}</td>
                    <td className="py-4 px-6 text-slate-600">{borrower.time_borrowed}</td>
                    <td className="py-4 px-6 text-slate-600">{borrower.time_return}</td>
                    <td className="py-4 px-6 text-slate-600">{borrower.no_hp}</td>
          ...
          <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
            ...
            {/* Form Section */}
              <form onSubmit={handleSubmit} className="flex-1 p-6 space-y-4 overflow-y-auto">
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2">Name Borrower</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2">Time Borrowed</label>
                  <input
                    type="text"
                    required
                    value={formData.time_borrowed}
                    onChange={(e) => setFormData({ ...formData, time_borrowed: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all"
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2">Time Return</label>
                  <input
                    type="text"
                    value={formData.time_return}
                    onChange={(e) => setFormData({ ...formData, time_return: e.target.value })}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all"
